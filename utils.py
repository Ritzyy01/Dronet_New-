...
def hard_mining_entropy(k, class_weights_dict):
    """
    Compute binary cross-entropy for collision evaluation and hard-mining.

    # Arguments
        k: Number of samples for hard-mining.
        class_weights_dict: Dictionary mapping class indices (0, 1) to weights.
    ...
    def custom_bin_crossentropy(y_true, y_pred):
        ...
        if n_samples_coll == 0:
            ...
            true_coll = y_true[:,1] # These are the labels (0 or 1)
            ...
            l_coll_base = K.binary_crossentropy(true_coll, pred_coll)
            
            # Apply weights
            l_coll_weighted = tf.multiply(l_coll_base, weights)

            # Mask loss to only include collision samples (t=0)
            l_coll = tf.multiply((1-t), l_coll_weighted)
