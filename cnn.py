...
def trainModel(train_data_generator, val_data_generator, model, initial_epoch):
    ...
    # --- CHANGED: Set beta to 1.0 (from 0) to enable collision loss ---
    model.alpha = tf.Variable(1.0, trainable=False, name='alpha', dtype=tf.float32)
    model.beta = tf.Variable(1.0, trainable=False, name='beta', dtype=tf.float32)
    # -------------------------------------------------------------------
    ...
    # --- NEW: Define class weights to fight imbalance ---
    # 0 = Safe, 1 = Crash.
    # Calculated as:
    # Total Samples: 1223 (Safe) + 353 (Crash) = 1576
    # Weight for Safe (Class 0): (1576 / 2) / 1223 = 0.64
    # Weight for Crash (Class 1): (1576 / 2) / 353 = 2.23
    class_weights_dict = {0: 0.64, 1: 2.23}
    # ----------------------------------------------------

    optimizer = optimizers.Adam(decay=1e-5)

    # Configure training process
    # --- CHANGED: Pass class weights directly to the loss function ---
    model.compile(loss=[utils.hard_mining_mse(model.k_mse),
                        utils.hard_mining_entropy(model.k_entropy, class_weights_dict)],
